    ...
    if (isLoading) {
      return (
        <div className="min-h-[70vh] flex flex-col items-center justify-center space-y-6 bg-white dark:bg-zinc-950">
          ...
          <p className={`text-zinc-500 font-bold uppercase tracking-widest text-[10px] ${lang === 'bn' ? 'font-bengali' : 'font-english'}`}>
              ...
              <div className="max-w-7xl mx-auto relative z-10">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-20 gap-8">
                  <div className="max-w-2xl">
                    <span className="text-red-600 font-black uppercase tracking-[0.3em] text-[10px] mb-6 block">Infrastructure</span>
                    <h2 className={`text-4xl md:text-6xl font-black leading-none text-zinc-900 dark:text-zinc-100 ${lang === 'bn' ? 'font-bengali' : 'font-english uppercase italic tracking-tight'}`}>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${theme === 'dark' ? 'bg-zinc-950 text-zinc-100' : 'bg-white text-zinc-900'}`}>
      ...
      <main className="pt-14 md:pt-16">
        <Suspense fallback={<div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-red-600" /></div>}>
          {renderPage()}
        </Suspense>
      </main>
