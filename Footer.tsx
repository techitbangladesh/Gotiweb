...
const Footer: React.FC<FooterProps> = ({ lang, theme }) => {
  ...
  return (
    <footer className="bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 py-12 px-6 border-t border-zinc-100 dark:border-zinc-900 transition-all duration-500 overflow-hidden relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              {SOCIAL_LINKS.map((social) => (
                <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-lg flex items-center justify-center text-zinc-400 hover:text-red-600 transition-all">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-3">
              <h3 className="text-[8px] font-black text-red-600 uppercase tracking-widest font-english">Support</h3>
              <div className="space-y-3">
                <div>
                  <p className="text-[7px] font-black text-zinc-400 uppercase tracking-widest font-english">Contact</p>
                  <p className="text-xs font-black font-english tracking-tight">{CONTACT_NUMBER}</p>
                </div>
                <div>
                  <p className="text-[7px] font-black text-zinc-400 uppercase tracking-widest font-english">Email</p>
                  <a href="mailto:info.techitnetwork@gmail.com" className="text-[10px] font-bold hover:text-red-600 transition-colors truncate block">info.techitnetwork@gmail.com</a>
                </div>
              </div>
            </div>
          </div>
